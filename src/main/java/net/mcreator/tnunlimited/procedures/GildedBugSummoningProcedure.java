...
public class GildedBugSummoningProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		if (entity == null)
			return;
		if ((entity instanceof LivingEntity _livEnt ? _livEnt.hasEffect(TnunlimitedModMobEffects.GILDED_BUG_BUFF.get()) : false)
				&& TnunlimitedModVariables.MapVariables.get(world).SessionLength >= 300) {
			if (world instanceof ServerLevel _level) {
				Entity entityToSpawn = new GildedBugEntity(TnunlimitedModEntities.GILDED_BUG.get(), _level);
