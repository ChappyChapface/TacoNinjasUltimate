		...
		if (itemstack.getOrCreateTag().getDouble("ammo") > 0) {
			...
			if (!(new Object() {
				...
				if (num >= 0) {
					...
					if (world instanceof ServerLevel _level)
						_level.sendParticles((SimpleParticleType) (TnunlimitedModParticleTypes.BULLET_SHELL.get()), (entity.getX()),
								(entity.getY() + 1), (entity.getZ()), 6, 0.2, 0.2, 0.2, 0.1);
				}
			} else if (new Object() {
				public boolean checkGamemode(Entity _ent) {
					if (_ent instanceof ServerPlayer _serverPlayer) {
