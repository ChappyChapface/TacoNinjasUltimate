...
public class SlothsPillowEffectProcedure {
	...
	private static void execute(@Nullable Event event, Entity entity, Entity sourceentity) {
		...
		if (((sourceentity.getCapability(TnunlimitedModVariables.PLAYER_VARIABLES_CAPABILITY, null)
				.orElse(new TnunlimitedModVariables.PlayerVariables())).Acc00).getItem() == TnunlimitedModItems.SLOTHS_PILLOW.get()
				|| ((sourceentity.getCapability(TnunlimitedModVariables.PLAYER_VARIABLES_CAPABILITY, null)
						.orElse(new TnunlimitedModVariables.PlayerVariables())).Acc01).getItem() == TnunlimitedModItems.SLOTHS_PILLOW.get()
				|| ((sourceentity.getCapability(TnunlimitedModVariables.PLAYER_VARIABLES_CAPABILITY, null)
						.orElse(new TnunlimitedModVariables.PlayerVariables())).Acc02).getItem() == TnunlimitedModItems.SLOTHS_PILLOW.get()) {
			if (Math.round(Math.random() * 10) >= 10 - (sourceentity instanceof LivingEntity _livEnt && _livEnt.hasEffect(MobEffects.LUCK)
				...
				if (entity instanceof LivingEntity _entity)
					_entity.addEffect(new MobEffectInstance(TnunlimitedModMobEffects.DROWSY.get(), 40, 0));
			}
		}
	}
}
