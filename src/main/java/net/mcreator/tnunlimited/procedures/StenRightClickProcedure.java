		...
		if (itemstack.getOrCreateTag().getDouble("ammo") > 0) {
			...
			if (entity instanceof Player _player)
				_player.getCooldowns().addCooldown(itemstack.getItem(),
						(int) (3 / (1 + EnchantmentHelper.getItemEnchantmentLevel(TnunlimitedModEnchantments.TRIGGER_HAPPY, itemstack) * 0.25)));
			if (entity instanceof LivingEntity _ent_sa && !_ent_sa.level.isClientSide()) {
				BulletProjectileEntity.shoot(_ent_sa.level, _ent_sa, _ent_sa.level.getRandom(), 3,
						(float) (1 + 0.15 * EnchantmentHelper.getItemEnchantmentLevel(TnunlimitedModEnchantments.CALIBER, itemstack)), 0);
			}
			...
			if (world instanceof ServerLevel _level)
				_level.sendParticles(TnunlimitedModParticles.BULLET_SHELL, (entity.getX()), (entity.getY() + 1), (entity.getZ()), 1, 0.2, 0.2, 0.2,
						0.3);
			if (EnchantmentHelper.getItemEnchantmentLevel(TnunlimitedModEnchantments.KICKBACK, itemstack) >= 1) {
				n = (((-2) * (1 + 0.25 * EnchantmentHelper.getItemEnchantmentLevel(TnunlimitedModEnchantments.CALIBER, itemstack)))
